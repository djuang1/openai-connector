...
public class OpenAIOperations {
	...
	public CompletionResult createCompletion(@Connection OpenAIConnection connection,
			...
			@Content @DisplayName("Stop") @Optional @Summary("Up to 4 sequences where the API will stop generating further tokens. The returned text will not contain the stop sequence.") List<String> stops,
		...
		CompletionRequest completionRequest = CompletionRequest.builder()
				...
				.bestOf(bestOf)
				.logitBias(LogitBiasParameters.toMap(logitBiases))
				.user(user)
				.build();
